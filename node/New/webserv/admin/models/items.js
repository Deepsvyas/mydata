        ...
        getUnits(Unit, function (units) {
            getGstslabs(Gstslabs, function (gstslabs) {
                getShops(Shop, function (shops) {
                    res.render('item/list', {page_title: "Items | Admin", categories : categories,units : units,gstslabs : gstslabs,shops : shops, item:true});
                });
            });
        });
    });
...
exports.addnew = function (req, res, next) {
    ...
    req.check('price', 'Price is required').isInt();
    req.check('available_qty', 'Quantity is required').isInt();
